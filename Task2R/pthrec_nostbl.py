...
for i in range(Ne):
    for j in range(1,N):
        if i == j or j in target or (nrnx[i]==3 and nrny[i]==3) or (nrnx[j]==3 and nrny[j]==3):
            continue
        if (nrnx[i],nrny[i],nrnx[j],nrny[j]) == (2,4,2,2) or (nrnx[i],nrny[i],nrnx[j],nrny[j]) == (2,4,3,2) or (nrnx[i],nrny[i],nrnx[j],nrny[j]) == (2,4,4,3):   
            continue
        dist = linalg.norm(array([nrnx[i]-nrnx[j],nrny[i]-nrny[j]]))
        if dist <= rng:
            if dist > 2 and i < target[0]:
                continue
            r = np.random.rand()
            if (i,j) in temp or (j,i) in temp:
                continue
            if i < target[0] and dist <= 2:
                temp.append((i,j))
            elif i in target:
                temp.append((j,i))
                ...
                if vdt < 1/1.4142 and vdt > -1/1.4143:
                    if ((nrnx[i]-3)*(nrny[j]-3) - (nrny[i]-3)*(nrnx[j]-3)) < 0:
                        if not(nrnx[j] < nrny[j] and nrny[j] < 6-nrnx[j]):
                            if (np.abs(nrnx[j]-nrny[j])+np.abs(nrnx[j]+nrny[j]-6))==4 and (np.abs(nrnx[i]-nrny[i])+np.abs(nrnx[i]+nrny[i]-6))==4:  
                                if dist <= 2:
                                    temp.append((i,j))
                            elif r < 0.5:
                                temp.append((i,j))
...
#input -> dopamine (post)
de2post = '''
    fstsp += (-fstsp + int(counter < 21)*int(counter>0))*(1-flag)
    flag = 1
'''
...
if np.nan not in array(G.v):
    with open("./preserve/pthrecsetup_nostbl{}.pkl".format(sys.argv[1]) ,mode='wb') as f:
        pickle.dump(ii1,f)
        pickle.dump(jj1,f)
        pickle.dump(nrnx,f)
        pickle.dump(nrny,f)
    ...
    with open("./preserve/pthrecresult_nostbl{}.pkl".format(sys.argv[1]) ,mode='wb') as f:
